...
function generateFlatObject(size) {
  const obj = {};
...
function generateMixedObject(size) {
  const obj = {
    id: 1,
    name: 'test',
    active: true,
...
function generateObjectWithDates(size) {
  const obj = {
...
function generateEventLog(size) {
  const events = [];
...
function mutate(obj, changeRatio = 0.1) {
